...
public class DL4JTest extends TestCase {
    ...
    public void test_dl4j_operation() {
        ...
        NDArray b = np.multiply(a, 2);
        b = b.divide(2);
        assertEquals(np.array(expected), b);

        double[][] expected2 = {{10.0, 11.0, 12.0}, {13.0, 14.0, 15.0}};
        assertEquals(np.array(expected2), np.array(expected).add(10));

        double[][] expected3 = {{10.0, 11.0, 12.0}, {103.0, 104.0, 105.0}};
        int[][] dat = {{10, 10, 10}, {100, 100, 100}};
        assertEquals(np.array(expected3), np.array(expected).add(dat));

        double[][] expected4 = {{10.0, 21.0, 32.0}, {13.0, 24.0, 35.0}};
        int[][] dat2 = {{10, 20, 30}};
        assertEquals(np.array(expected4), np.array(expected).add(dat2));
    }
    ...
    public void test_dl4j_operation_polynomial() {
        ...
        int[][] dat = {{10, 20, 30}};
        NDArray ret = np.array(input).multiply(dat).divide(5);
        double[][] expected = {{0.0, 4.0, 12.0}, {6.0, 16.0, 30.0}};
        assertEquals(np.array(expected), ret);
    }
    ...
    public void test_dl4j_gen_data() {
        NDArray x1 = np.linspace(0, 9, 10).V();
        NDArray x2 = np.linspace(4, 13, 10).V();
        NDArray x = np.concatenate(x1, x2);
        System.out.println(x.T());

        NDArray v = np.array(new int[]{3, 4});
        NDArray y = np.dot(x.T(), v.T());
        double[] expected1 = {16.0, 23.0, 30.0, 37.0, 44.0, 51.0, 58.0, 65.0, 72.0, 79.0};
        assertEquals(np.array(expected1), y);

        v = np.array(new int[]{3, 4}).V();
        y = np.dot(x.T(), v.T());
        double[][] expected2 = {{16.0}, {23.0}, {30.0}, {37.0}, {44.0}, {51.0}, {58.0}, {65.0}, {72.0}, {79.0}};
        assertEquals(np.array(expected2), y);
    }
